...
public class MavenClassPathParser {
    ...
    protected String getClasspathString() {
        ...
        if (!isCacheValid(classpathCache)) {
    ...
    private boolean isCacheValid(File classpathCache) {
        File pom = new File(pomDirectory, "pom.xml");

        if (classpathCache.lastModified() < pom.lastModified()) {
            return false;
        }

        File parentPom = new File(new Configuration().getProjectParentPomDirectory().getValue(), "pom.xml");

        if (classpathCache.lastModified() < parentPom.lastModified()) {
            return false;
        }
